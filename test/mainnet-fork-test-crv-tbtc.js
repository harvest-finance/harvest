...
if (process.env.MAINNET_FORK) {
  ...
  const CRVStrategyTBTCMixedMainnet = artifacts.require("CRVStrategyTBTCMixedMainnet");
  ...
  contract("Mainnet Curve TBTC", function (accounts) {
    describe(`Curve TBTC`, function () {
      // external contracts
      let underlying;

      // external setup
      let underlyingWhale = MFC.TBTC_MIXED_WHALE_ADDRESS;
      ...
      async function setupCoreProtocol() {
        ...
        strategy = await CRVStrategyTBTCMixedMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );
      ...
      it("A farmer investing TBTC mixed token", async function () {
