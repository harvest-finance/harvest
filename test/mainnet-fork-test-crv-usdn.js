...
if (process.env.MAINNET_FORK) {
  ...
  contract("Mainnet Curve USDN", function (accounts) {
    describe(`Curve USDN`, function () {
      // external contracts
      let underlying;
      let ctoken;
      ...
      async function setupCoreProtocol() {
        ...
        strategy = await CRVStrategyUSDNMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );
      ...
      it("A farmer investing usdn pool", async function () {
