...
if ( process.env.MAINNET_E2E ) {
  ...
  // Some contracts used in the E2E test are not finalized and currently not present in the repo
  // so the test is disabled
  contract.skip("Mainnet End-to-end test", function(accounts){
    describe("basic settings", function (){
      ...
      */
      ...
      async function setupInternalYcrvVault() {
        ...
        ycrvInternalVault = await Vault.new(storage.address, ycrv.address, 98, 100, {
          from: governance,
        });
      ...
      async function setupYcrvVault() {
        // ycrv Vault SNX
        ycrvVault = await Vault.new(storage.address, ycrv.address, 100, 100, {from: governance});
