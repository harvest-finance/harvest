...
if (process.env.MAINNET_FORK) {
  ...
  const CRVStrategyHBTCMainnet = artifacts.require("CRVStrategyHBTCMainnet");
  ...
  contract("Mainnet Curve HBTC", function (accounts) {
    describe(`Curve HBTC`, function () {
      // external contracts
      let underlying;

      // external setup
      let underlyingWhale = MFC.HBTC_WHALE_ADDRESS;
      ...
      async function setupExternalContracts() {
        underlying = await IERC20.at(MFC.HBTC_ADDRESS);
      }
      ...
      async function setupCoreProtocol() {
        ...
        strategy = await CRVStrategyHBTCMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );

        await strategy.setSellFloorCrv(0, { from: governance });
      ...
      it("A farmer investing HBTC mixed token", async function () {
