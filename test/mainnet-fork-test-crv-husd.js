...
if (process.env.MAINNET_FORK) {
  ...
  const CRVStrategyHUSDMainnet = artifacts.require("CRVStrategyHUSDMainnet");
  ...
  contract("Mainnet Curve HUSD", function (accounts) {
    describe(`Curve HUSD`, function () {
      // external contracts
      let underlying;

      // external setup
      let underlyingWhale = MFC.HUSD_POOL_WHALE_ADDRESS;
      ...
      async function setupCoreProtocol() {
        ...
        strategy = await CRVStrategyHUSDMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );
      ...
      it("A farmer investing HUSD pool", async function () {
