...
if (process.env.MAINNET_FORK) {
  ...
  contract("Mainnet Swerve DAI", function (accounts) {
    describe("Swerve savings", function () {
      ...
      // external setup
      let wbtcWhale = MFC.DAI_WHALE_ADDRESS;
      ...
      async function setupExternalContracts() {
        wbtc = await IERC20.at(MFC.DAI_ADDRESS);
      ...
      async function setupCoreProtocol() {
        ...
        feeRewardForwarder = await FeeRewardForwarder.new(storage.address, wbtc.address, MFC.UNISWAP_V2_ROUTER02_ADDRESS, { from: governance });
