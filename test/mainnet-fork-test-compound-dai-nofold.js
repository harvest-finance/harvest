...
if ( process.env.MAINNET_FORK ) {
  ...
  const CompoundNoFoldStrategyDAIMainnet = artifacts.require("CompoundNoFoldStrategyDAIMainnet");
  ...
  contract("Mainnet Compound DAI Strategy", function(accounts){
    describe("Mainnet Compound DAI earnings", function (){
      ...
      async function setupCoreProtocol() {
        ...
        // set up the strategy
        strategy = await CompoundNoFoldStrategyDAIMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );
