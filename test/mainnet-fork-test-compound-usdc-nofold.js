...
if ( process.env.MAINNET_FORK ) {
  ...
  const CompoundNoFoldStrategyUSDCMainnet = artifacts.require("CompoundNoFoldStrategyUSDCMainnet");
  ...
  contract("Mainnet Compound USDC Strategy", function(accounts){
    describe("Mainnet Compound USDC earnings", function (){
      ...
      async function setupCoreProtocol() {
        ...
        // set up the strategy
        strategy = await CompoundNoFoldStrategyUSDCMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );
