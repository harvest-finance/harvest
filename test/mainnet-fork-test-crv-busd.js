...
if (process.env.MAINNET_FORK) {
  ...
  const CRVStrategyBUSDMainnet = artifacts.require("CRVStrategyBUSDMainnet");
  ...
  contract("Mainnet Curve BUSD", function (accounts) {
    describe(`Curve BUSD`, function () {
      ...
      // external setup
      let underlyingWhale = MFC.BUSD_POOL_WHALE_ADDRESS;
      ...
      async function setupExternalContracts() {
        underlying = await IERC20.at(MFC.BUSD_POOL_ADDRESS);
      ...
      async function setupCoreProtocol() {
        ...
        strategy = await CRVStrategyBUSDMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );
      ...
      it("A farmer investing busd pool", async function () {
