...
if ( process.env.MAINNET_FORK ) {
  ...
  const SNXRewardKittenStrategy = artifacts.require("SNXRewardKittenStrategy");
  ...
  contract.only("Mainnet Kitten", function(accounts){
    describe("Kitten mining", function (){
      ...
      async function setupExternalContracts() {
        ...
        kif = await IERC20.at(MFC.KIF_ADDRESS);
      }
      ...
      async function setupCoreProtocol() {
        ...
        const feeRewardForwarder = await FeeRewardForwarder.new(storage.address, MFC.UNISWAP_V2_ROUTER02_ADDRESS, { from: governance });
        ...
        // set up the strategy
        strategy = await SNXRewardKittenStrategy.new(
          storage.address,
          weth.address,
          vault.address,
          MFC.KITTEN_WETH_RewardPool_ADDRESS,
          kif.address,
          { from: governance }
        );

        await strategy.setLiquidationRoute([MFC.KIF_ADDRESS, MFC.WETH_ADDRESS], {from: governance});
