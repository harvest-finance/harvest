...
if ( process.env.MAINNET_FORK ) {
  ...
  contract("Mainnet IDLE USDT Strategy", function(accounts){
    describe("Mainnet IDLE USDT earnings", function (){
      ...
      async function setupCoreProtocol() {
        ...
        // DAI vault
        vault = await Vault.at("0x053c80eA73Dc6941F518a68E2FC52Ac45BDE7c9C");

        // set up the strategy
        strategy = await IdleStrategyUSDTMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );
