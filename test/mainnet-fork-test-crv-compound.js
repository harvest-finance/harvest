...
if (process.env.MAINNET_FORK) {
  ...
  const CRVStrategyCompoundMainnet = artifacts.require("CRVStrategyCompoundMainnet");
  ...
  contract("Mainnet Curve Compound", function (accounts) {
    describe(`Curve Compound`, function () {
      // external contracts
      let underlying;
      let ctoken;

      // external setup
      let underlyingWhale = MFC.COMPOUND_POOL_WHALE_ADDRESS;
      ...
      async function setupExternalContracts() {
        underlying = await IERC20.at(MFC.COMPOUND_POOL_ADDRESS);
        ctoken = await IERC20.at("0x5d3a536E4D6DbD6114cc1Ead35777bAB948E3643");
      }
      ...
      async function setupCoreProtocol() {
        ...
        strategy = await CRVStrategyCompoundMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );
      ...
      it("A farmer investing curve compound pool", async function () {
