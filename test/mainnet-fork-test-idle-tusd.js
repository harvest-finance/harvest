...
if ( process.env.MAINNET_FORK ) {
  ...
  const IdleStrategyTUSDMainnet = artifacts.require("IdleStrategyTUSDMainnet");
  ...
  contract("Mainnet IDLE TUSD Strategy", function(accounts){
    describe("Mainnet IDLE TUSD earnings", function (){

      // external contracts
      let underlying;

      // external setup
      let underlyingWhale = MFC.TUSD_WHALE_ADDRESS;
      ...
      async function setupExternalContracts() {
        underlying = await IERC20.at(MFC.TUSD_ADDRESS);
      }
      ...
      async function setupCoreProtocol() {
        ...
        // DAI vault
        vault = await Vault.at("0x7674622c63Bee7F46E86a4A5A18976693D54441b");

        // set up the strategy
        strategy = await IdleStrategyTUSDMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );
