...
if ( process.env.MAINNET_FORK ) {
  ...
  contract.only("Mainnet Vault Migration", function(accounts){
    describe("ETH-USDC Vault", function (){
      let governance;
      ...
      const farmerBalance =  "60000000000000";
      ...
      async function setupExternalContracts() {
        oldVault = await Vault.at(MFC.VaultUNI_LP_WETH_USDC);
        newVault = await Vault.at(MFC.ProxiedVault_UNI_LP_WETH_USDC);
        migrator = await RewardPool.at(MFC.Proxied_UNI_LP_WETH_USDCPool);
        vaultMigrationStrategy = await VaultMigratorStrategy.at(MFC.MigrationStrategy_UNI_LP_WETH_USDC);
      }
      ...
      it("migration", async function () {
        ...
        Utils.assertApproxBNEq(newDepositsInNewVault.minus(originalDepositsInNewVault), oldUnderlyingBalance, "1000000", "New vault must have all the underlying");
