...
if ( process.env.MAINNET_FORK ) {
  ...
  contract("Mainnet IDLE WBTC Strategy", function(accounts){
    describe("Mainnet IDLE WBTC earnings", function (){

      // external contracts
      let underlying;

      // external setup
      let underlyingWhale = MFC.WBTC_WHALE_ADDRESS;
      ...
      async function setupExternalContracts() {
        underlying = await IERC20.at(MFC.WBTC_ADDRESS);
      }
      ...
      async function setupCoreProtocol() {
        ...
        // DAI vault
        vault = await Vault.at("0x5d9d25c7C457dD82fc8668FFC6B9746b674d4EcB");

        // set up the strategy
        strategy = await IdleStrategyWBTCMainnet.new(
          storage.address,
          vault.address,
          { from: governance }
        );
