...
contract Vault is ERC20, ERC20Detailed, IVault, Controllable {
  ...
  */
  ...
  function setStrategy(address _strategy) public onlyControllerOrGovernance {
    ...
    require(IStrategyV2(_strategy).underlying() == address(underlying), "Vault underlying must match Strategy underlying");
    require(IStrategyV2(_strategy).vault() == address(this), "the strategy does not belong to this vault");
  ...
  */
  ...
  function _deposit(uint256 amount, address sender, address beneficiary) internal {
    ...
    if (totalSupply() != 0) {
        uint256 slippage = strategy.depositSlippageCheck(amount);
        toMint = amount
          .mul(totalSupply())
